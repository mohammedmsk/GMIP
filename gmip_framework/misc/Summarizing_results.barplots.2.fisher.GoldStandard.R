...
data=data[data$ID=="top_0500_reprio_genes",]
bp="barplots_fisher_top0500_reprio_GS"
#data=data[data$ID=="top_0500_gwas_genes",]
#bp="barplots_fisher_top0500_gwas_genes"
...
generate_combined_barplot_per_gwas <- function(gwas) {
  ...
  for (strategy in unique_strategies) {
    ...
    for (method in method_names) {
      ...
      p <- ggplot(method_data, aes(x = feature_name, y = Count)) +
        ...
        labs(title = paste(method, "-", strategy), x = "", y = "Overlap Count") +
        theme_minimal() +
  ...
  ggsave(filename = paste0(bp, "/", gwas, "_combined_barplot_all_strategies.png"), 
