...
file_pattern="*.ypred_MLmeasures.tsv"
...
combined_results <- rbindlist(lapply(files_info$file_path, function(file) {
  dt <- fread(file)
  dt=as.data.table(dcast(melt(dt, id.vars = "Measure"), variable ~ Measure)[,-1])
...
data=combined_results[,c("GWAS_name", "feature_name", "Method_name", "CV_strategy", "binary_truth.AUC", "binary_truth.LiftAUC")]


# Create directory for barplots if it doesn't exist
if (!dir.exists("barplots_MLMeasure_AUC")) {
  dir.create("barplots_MLMeasure_AUC")
}
...
generate_combined_barplot_per_gwas <- function(gwas) {
  ...
  for (strategy in unique_strategies) {
    ...
    y_max <- max(subset_data$binary_truth.AUC, na.rm = TRUE) * 1.1  # Add 10% cushion to ensure visibility of asterisks
    ...
    for (method in method_names) {
      ...
      p <- ggplot(method_data, aes(x = feature_name, y = binary_truth.AUC)) +
        ...
        labs(title = paste(method, "-", strategy), x = "", y = "AUROC") +
        theme_minimal() +
  ...
  ggsave(filename = paste0("barplots_MLMeasure_AUC/", gwas, "_combined_barplot_all_strategies.png"), 
