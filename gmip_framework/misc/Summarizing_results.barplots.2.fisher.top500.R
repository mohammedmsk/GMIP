...
file_pattern="*.ypred_gwas.overlap.tsv"
...
data=combined_results[,c("GWAS_name", "feature_name", "Method_name", "CV_strategy", "p.adjust", "Count", "ID")]
data=data[data$ID=="top_0500_gwas_genes__ypred_top500genes",]

# Create directory for barplots if it doesn't exist
if (!dir.exists("barplots_fisher_top0500")) {
  dir.create("barplots_fisher_top0500")
}
...
generate_combined_barplot_per_gwas <- function(gwas) {
  ...
  for (strategy in unique_strategies) {
    ...
    y_max <- max(subset_data$Count, na.rm = TRUE) * 1.1  # Add 10% cushion to ensure visibility of asterisks
    ...
    for (method in method_names) {
      ...
      p <- ggplot(method_data, aes(x = feature_name, y = Count)) +
  ...
  ggsave(filename = paste0("barplots_fisher_top0500/", gwas, "_combined_barplot_all_strategies.png"), 
